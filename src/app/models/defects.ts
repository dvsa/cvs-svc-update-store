import {DynamoDbImage} from "../../infrastructure/dynamo/dynamodb-images";
...
export const parseDefect = (image: DynamoDbImage): Defect => {
    return {
        ...
        additionalInformation: parseDefectAdditionalInformation(image.getMap("additionalInformation")),
        ...
        deficiencyCategory: image.getString("deficiencyCategory") as DeficiencyCategory,
...
export const parseDefectAdditionalInformation = (image?: DynamoDbImage): Maybe<DefectAdditionalInformation> => {
    if (!image) {
        return undefined;
    }

    return {
        location: parseDefectAdditionalInformationLocation(image.getMap("location")),
        notes: image.getString("notes")
    };
};

export const parseDefectAdditionalInformationLocation = (image?: DynamoDbImage): Maybe<DefectAdditionalInformationLocation> => {
    if (!image) {
        return undefined;
    }
